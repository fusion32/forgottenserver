...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if ((attr = node.attribute("chance"))) {
		...
		if (chance > 100) {
			chance = 100;
			LOG_WARN("{}: spell {} chance is greater than 100%", description, name);
		}
		sb.chance = chance;
	} else if (boost::algorithm::to_lower_copy(name) != "melee") {
		LOG_WARN("{}: missing chance attribute for non-melee spell {}", description, name);
	}
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			if ((attr = node.attribute("speedchange"))) {
				...
				if (minSpeedChange == 0) {
					LOG_ERR("{}: missing speedchange/minspeedchange value", description);
					return false;
				}
			...
			if (minSpeedChange < -1000) {
				LOG_ERR("{}: you cannot reduce a creatures speed below -1000 (100%)", description);
				minSpeedChange = -1000;
			}
			...
			LOG_ERR("{}: unknown spell name {}", description, name);
			return false;
		}
				...
				if (caseInsensitiveEqual(value, "shooteffect")) {
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (shoot != CONST_ANI_NONE) {
							...
							LOG_WARN("{}: invalid shoot effect {}", description, attr.as_string());
						}
					}
					...
					if ((attr = attributeNode.attribute("value"))) {
						...
						if (effect != CONST_ME_NONE) {
							...
							LOG_WARN("{}: invalid area effect {}", description, attr.as_string());
						}
					}
				} else {
					LOG_WARN("{}: invalid effect type {}", description, attr.as_string());
				}
			}
		}
	}
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		...
		if (!combatSpellPtr->loadScript("data/" + std::string{g_spells->getScriptBaseName()} + "/scripts/" +
		                                spell->scriptName)) {
			LOG_ERR("{}: cannot find file", description);
			return false;
		}
		...
		if (tmpName == "melee") {
			...
			if (spell->combatType == COMBAT_UNDEFINEDDAMAGE) {
				LOG_WARN("{}: spell has undefined damage", description);
				combat->setParam(COMBAT_PARAM_TYPE, COMBAT_PHYSICALDAMAGE);
			}
			...
			if (minSpeedChange < -1000) {
				LOG_WARN("{}: you cannot reduce a creatures speed below -1000 (100%)", description);
				minSpeedChange = -1000;
			}
			...
			if (spell->conditionType == CONDITION_NONE) {
				LOG_ERR("{}: no condition set for spell {}", description, spell->name);
			}
		} else if (tmpName == "strength") {
			//
		} else if (tmpName == "effect") {
			...
			LOG_ERR("{}: unknown spell {}", description, spell->name);
		}
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if (!monsterNode) {
		LOG_ERR("{}: missing monster node", file);
		return nullptr;
	}

	pugi::xml_attribute attr;
	if (!(attr = monsterNode.attribute("name"))) {
		LOG_ERR("{}: missing monster name", file);
		return nullptr;
	}
	...
	if ((attr = monsterNode.attribute("race"))) {
		...
		if (tmpStrValue == "venom" || tmpInt == 1) {
			...
			LOG_WARN("{}: invalid race {}", file, attr.as_string());
		}
	}
	...
	if ((attr = monsterNode.attribute("script"))) {
		...
		if (scriptInterface->loadFile("data/monster/scripts/" + script) == 0) {
			...
			LOG_ERR("unable to load script {}: {}", script, scriptInterface->getLastLuaError());
		}
	}
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			if (caseInsensitiveEqual(attrName, "summonable")) {
				...
				if (staticAttack > 100) {
					LOG_WARN("{}: staticattack greater than 100", file);
					staticAttack = 100;
				}
				...
				if (targetDistance < 1) {
					targetDistance = 1;
					LOG_WARN("{}: targetdistance less than 1", file);
				}
				...
				LOG_WARN("{}: unknown flag attribute {}", file, attrName);
			}
		}
	...
	if ((node = monsterNode.child("bestiary"))) {
		...
		if ((attr = node.attribute("difficulty"))) {
			...
			if (tmpStrValue == "harmless") {
				...
				LOG_WARN("{}: unknown difficulty {}", file, attr.as_string());
			}
		}
		...
		if (!isValidBestiaryInfo(mType->bestiaryInfo)) {
			mType->bestiaryInfo = {};
			LOG_WARN("{}: invalid bestiary info for {}", file, mType->name);
		} else {
	...
	if (mType->info.manaCost == 0 && (mType->info.isSummonable || mType->info.isConvinceable)) {
		LOG_WARN("{}: manaCost missing or zero on monster with summonable and/or convinceable flags", file);
	}
	...
	if ((node = monsterNode.child("targetchange"))) {
		if ((attr = node.attribute("speed")) || (attr = node.attribute("interval"))) {
			...
			LOG_WARN("{}: missing targetchange speed", file);
		}
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				LOG_WARN("{}: targetchange is greater than 100%", file);
			}
			mType->info.changeTargetChance = chance;
		} else {
			LOG_WARN("{}: missing targetchange chance", file);
		}
	}
	...
	if ((node = monsterNode.child("look"))) {
		if ((attr = node.attribute("type"))) {
			...
			LOG_WARN("{}: missing look type/typeex", file);
		}
	...
	if ((node = monsterNode.child("attacks"))) {
		for (auto attackNode : node.children()) {
			...
			if (deserializeSpell(attackNode, sb, monsterName)) {
				...
				LOG_WARN("{}: unable to load spell", file);
			}
		}
	}
		...
		for (auto immunityNode : node.children()) {
			if ((attr = immunityNode.attribute("name"))) {
				...
				if (tmpStrValue == "physical") {
					...
					LOG_WARN("{}: invalid immunity {}", file, attr.as_string());
				}
				...
				LOG_WARN("{}: unknown immunity", file);
			}
		}
	}
	...
	if ((node = monsterNode.child("voices"))) {
		if ((attr = node.attribute("speed")) || (attr = node.attribute("interval"))) {
			...
			LOG_WARN("{}: missing voices speed", file);
		}
		...
		if ((attr = node.attribute("chance"))) {
			...
			if (chance > 100) {
				chance = 100;
				LOG_WARN("{}: yell chance is greater than 100%", file);
			}
			mType->info.yellChance = chance;
		} else {
			LOG_WARN("{}: missing voices chance", file);
		}
		...
		for (auto voiceNode : node.children()) {
			...
			if ((attr = voiceNode.attribute("sentence"))) {
				...
				LOG_WARN("{}: missing voice sentence", file);
			}
	...
	if ((node = monsterNode.child("loot"))) {
		for (auto lootNode : node.children()) {
			...
			if (loadLootItem(lootNode, lootBlock)) {
				...
				LOG_WARN("{}: unable to load loot", file);
			}
		}
	}
		...
		for (auto elementNode : node.children()) {
			if ((attr = elementNode.attribute("physicalPercent"))) {
				...
				if (mType->info.damageImmunities & COMBAT_PHYSICALDAMAGE) {
					LOG_WARN("{}: same element \"physical\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_ICEDAMAGE) {
					LOG_WARN("{}: same element \"ice\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_EARTHDAMAGE) {
					LOG_WARN("{}: same element \"earth\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_FIREDAMAGE) {
					LOG_WARN("{}: same element \"fire\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_ENERGYDAMAGE) {
					LOG_WARN("{}: same element \"energy\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_HOLYDAMAGE) {
					LOG_WARN("{}: same element \"holy\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_DEATHDAMAGE) {
					LOG_WARN("{}: same element \"death\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_DROWNDAMAGE) {
					LOG_WARN("{}: same element \"drown\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_LIFEDRAIN) {
					LOG_WARN("{}: same element \"lifedrain\" on immunity and element tags.", file);
				}
				...
				if (mType->info.damageImmunities & COMBAT_MANADRAIN) {
					LOG_WARN("{}: same element \"manadrain\" on immunity and element tags.", file);
				}
			} else {
				LOG_WARN("{}: unknown element", file);
			}
		}
	}
	...
	if ((node = monsterNode.child("summons"))) {
		if ((attr = node.attribute("maxSummons"))) {
			...
			LOG_WARN("{}: missing \"maxSummons\" attribute", file);
		}
		...
		for (auto summonNode : node.children()) {
			...
			if ((attr = summonNode.attribute("chance"))) {
				...
				if (chance > 100) {
					chance = 100;
					LOG_WARN("{}: summon chance is greater than 100%", file);
				}
			}
			...
			for (const auto& attributeNode : summonNode.children()) {
				if ((attr = attributeNode.attribute("key"))) {
					...
					if (caseInsensitiveEqual(value, "mastereffect")) {
						if ((attr = attributeNode.attribute("value"))) {
							masterEffect = getMagicEffect(boost::algorithm::to_lower_copy<std::string>(attr.as_string()));
							if (masterEffect == CONST_ME_NONE) {
								LOG_WARN("{}: unknown summon master effect {}", file, attr.as_string());
							}
						}
						...
						if ((attr = attributeNode.attribute("value"))) {
							...
							if (effect == CONST_ME_NONE) {
								...
								LOG_WARN("{}: unknown summon effect {}", file, attr.as_string());
							}
						}
					} else {
						LOG_WARN("{}: invalid summon effect type {}", file, attr.as_string());
					}
				}
			}
			...
			if ((attr = summonNode.attribute("name"))) {
				...
				LOG_WARN("{}: missing summon name", file);
			}
		}
	}
	...
	if ((node = monsterNode.child("script"))) {
		for (auto eventNode : node.children()) {
			if ((attr = eventNode.attribute("name"))) {
				...
				LOG_WARN("{}: missing name for script event", file);
			}
		}
	}
...
bool Monsters::loadLootItem(const pugi::xml_node& node, LootBlock& lootBlock)
{
	...
	if ((attr = node.attribute("id"))) {
		...
		if (it.name.empty()) {
			LOG_WARN("invalid loot item id {}", id);
			return false;
		}
		...
		if (ids.first == Item::items.nameToItems.cend()) {
			LOG_WARN("invalid loot item name \"{}\"", name);
			return false;
		}
		...
		if (std::next(ids.first) != ids.second) {
			LOG_WARN("non-unique loot item \"{}\"", name);
			return false;
		}
	...
	if ((attr = node.attribute("chance")) || (attr = node.attribute("chance1"))) {
		...
		if (lootChance > static_cast<int32_t>(MAX_LOOTCHANCE)) {
			LOG_WARN("loot chance {} is too large (max value: {})", lootChance, MAX_LOOTCHANCE);
			lootChance = MAX_LOOTCHANCE;
		}
		lootBlock.chance = lootChance;
	} else {
...
{
	if (info.raceId == 0) {
		LOG_WARN("race id can't be 0");
		return false;
	}

	if (info.className.empty()) {
		LOG_WARN("class name can't be empty");
		return false;
	}

	if (info.prowess == 0 || info.expertise == 0 || info.mastery == 0) {
		LOG_WARN("prowess, expertise and mastery can't be 0");
		return false;
	}

	if (info.prowess >= info.expertise || info.expertise >= info.mastery) {
		LOG_WARN("prowess must be lower than expertise and expertise must be lower than mastery");
		return false;
	}

	if (info.difficulty > BESTIARY_MAX_DIFFICULTY) {
		LOG_WARN("difficulty can't be higher than {}", BESTIARY_MAX_DIFFICULTY);
		return false;
	}

	if (info.occurrence > BESTIARY_MAX_OCCURRENCE) {
		LOG_WARN("occurrence can't be higher than {}", BESTIARY_MAX_OCCURRENCE);
		return false;
	}
