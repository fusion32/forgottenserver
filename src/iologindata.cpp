...
bool IOLoginData::preloadPlayer(Player* player)
{
	...
	if (!group) {
		LOG_ERR("{} has invalid group id {}", player->name, result->getNumber<uint16_t>("group_id"));
		return false;
	}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if (!player->setVocation(result->getNumber<uint16_t>("vocation"))) {
		LOG_ERR("{} has invalid vocation id {}", player->name, result->getNumber<uint16_t>("vocation"));
		return false;
	}
	...
	if (!town) {
		LOG_ERR("{} has invalid town id {}", player->name, result->getNumber<uint32_t>("town_id"));
		return false;
	}
	...
	if ((result = db.storeQuery(
		...
		if (!guild) {
			...
			if (guild) {
				...
				LOG_WARN("{} has invalid guild id {}", player->name, guildId);
			}
		}
	...
	do {
		...
		if (item) {
			if (!item->unserializeAttr(propStream)) {
				LOG_WARN("failed to deserialize item attributes");
			}
