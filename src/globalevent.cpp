...
bool GlobalEvents::registerEvent(Event_ptr event, const pugi::xml_node&)
{
	...
	LOG_WARN("duplicate globalevent name {}", globalEvent->getName());
	return false;
}
...
void GlobalEvents::think()
{
	...
	for (auto& it : thinkMap) {
		...
		if (!globalEvent.executeEvent()) {
			LOG_ERR("failed to execute event {}", globalEvent.getName());
		}
...
bool GlobalEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (!nameAttribute) {
		LOG_ERR("missing globalevent name");
		return false;
	}
	...
	if ((attr = node.attribute("time"))) {
		...
		if (hour < 0 || hour > 23) {
			LOG_ERR("{}: invalid hour {}", name, attr.as_string());
			return false;
		}
		...
		if (params.size() > 1) {
			min = params[1];
			if (min < 0 || min > 59) {
				LOG_ERR("{}: invalid minute {}", name, attr.as_string());
				return false;
			}
			...
			if (params.size() > 2) {
				sec = params[2];
				if (sec < 0 || sec > 59) {
					LOG_ERR("{}: invalid second {}", name, attr.as_string());
					return false;
				}
			}
		}
		...
		if (caseInsensitiveEqual(value, "startup")) {
			...
			LOG_ERR("{}: invalid globalevent type {}", name, attr.as_string());
			return false;
		}
		...
		LOG_ERR("{}: missing interval attribute", name);
		return false;
	}
	return true;
}
